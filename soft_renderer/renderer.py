...
class SoftRenderer(nn.Module):
    ...
    def render_mesh(self, mesh, mode=None):
        ...
        mesh = self.lighting(mesh)
        if (type(mode) is dict) and 'save_obj' in mode:
            mesh.save_obj(mode['save_obj'], save_texture = True)
            print(mode['save_obj'])
        mesh = self.transform(mesh) #applies projection that distorts mesh symmetry 
        if (type(mode) is dict) and 'save_obj' in mode:
            transf_obj_name = os.path.splitext(mode['save_obj'])[0] + '_transformed.obj'
            mesh.save_obj(transf_obj_name , save_texture = True)
            print(transf_obj_name)
