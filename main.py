...
import utils.debug_utils as du  #to output images in different formats
#from utils.geometry import reflectX,  save_obj_with_uv
import utils.geometry  as gu
from utils.mesh import convert_3d_to_uv_coordinates
...
class MultiShapePredictor():
    def __init__(self, args, DEBUG, now):
        ...
        self.save_mesh_in_test = True 
        self.save_input_in_test = True

        self.save_all_mean_shape = False
        ...
        if self.args.dataset_name == 'pascal':
            if self.args.is_training:
                self.dataset_train = PascalDataset(init_mode='train',
                                                   ...
                                                   aug_mode=not self.args.disable_aug, cmr_mode=self.args.cmr_mode, data_index_name  = self.args.data_index_name)
                self.dataset_train.train()
                self.dataset_eval = PascalDataset(init_mode='eval',
                                                  dataset_dir=self.args.dataset_dir, classes=self.classes,
                                                  img_size=self.args.img_size, demo_mode=self.args.demo,
                                                  aug_mode=False, cmr_mode=self.args.cmr_mode, data_index_name  = self.args.data_index_name)
    ...
    def define_model(self):
        ...
        self.G_net = MCMRNet(self.img_size, self.args, nz_feat=self.args.nz_feat,
                             ...
                             texture_type=self.args.texture_type, enable_shape_symmetrization = self.args.symmetrize)
        ...
        #save useful data 
        self.V_sphere = self.G_net.icosa_sphere_v.clone().cpu().numpy()
        self.V_sphere[:,1] = -self.V_sphere[:,1]
        self.F_sphere = self.G_net.icosa_sphere_f.clone().cpu().numpy() 
        ##change orientation to get Outward normals 
        #self.F_sphere = np.stack((self.F_sphere[:,0], self.F_sphere[:,2], self.F_sphere[:,1]),axis = 1)
        self.UV=(1-convert_3d_to_uv_coordinates(self.V_sphere))/2
    ...
    def define_optimizer(self, load_chekpoint=True):
        ...
        if load_chekpoint and self.args.pretrained_weights.stem != '':
            self.G_optimizer.load_state_dict(self.checkpoint['G_optimizer']) #load weights
    ...
    def test_subdivision(self,  verts_, faces_, texture,  Rt_pred, K_pred, scale_pred, Rt_gt, K_gt):
        self.visual_renderer = SOFTRAS_renderer(img_size=self.args.img_size, camera_mode='projection',
                                                orig_size=self.args.img_size, background=[1, 1, 1],
                                                texture_type=self.args.texture_type, anti_aliasing=True,
                                                clamp_lighting=True)

        correct_DR_texture = False #true=> DR renders correct textute but in obj file the texture will be flipped
        if correct_DR_texture:
            verts = verts_
            faces = faces_
        else:
            verts = gu.reflectY(verts_) #correct meshlab texture, but wrong SoftRas DR texture 
            faces = gu.invert_face_orientation(faces_)
        ...
        # pred cam
        self.visual_renderer.set_camera(K_pred, Rt_pred)#set camera transform form predcted camera translation and rotation (K_pred,Rt_pred)
        ...
        #color = torch.tensor([156, 199, 234.], device=self.device) / 255.
        color = torch.tensor([190., 190., 190.], device=self.device) / 255.
        default_texture = torch.ones((1, faces.shape[1], 36, 3)).float().to(self.device)* color
        img_pred_shape, _ = self.visual_renderer(verts, faces, default_texture) #render predicted mesh without texture
        #img_pred_shape = (img_pred_shape.squeeze(0).detach().cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
        du.tensor_to_batch(SDR_to_numpy(img_pred_shape), "img_pred_shape")
        img_pred_texture, _ = self.visual_renderer(verts, faces, texture)
        du.tensor_to_batch(SDR_to_numpy(img_pred_texture), "img_pred_texture")

        unwrapp_tex_file =  './debug/original_mesh_texture.png'
        cv2.imwrite(unwrapp_tex_file,  SDR_to_numpy(self.G_net.albedo_predictor.uvimage_pred))
        print('writing ' + unwrapp_tex_file)
        obj_uv_file = './debug/original_mesh.obj'
        V = verts.cpu().squeeze().numpy()
        #V[:,1] = -V[:,1]
        #save_obj_with_uv(obj_uv_file,V, self.F_sphere, UV, unwrapp_tex_file)
        gu.save_obj_with_uv(obj_uv_file,V, self.F_sphere, self.UV, 'original_mesh_texture.png')
        print('writing ' + obj_uv_file)

               
        #V_sub, F_sub, tex_pred  = gu.subdivide_mesh_wrapper(igl.upsample, verts, faces, self.V_sphere, self.G_net.icosa_sphere_f, self.G_net.albedo_predictor.uvimage_pred) 
        #subdiv_func = igl.upsample
        #subdiv_func = igl.loop
        subdiv_func = gu.centroid_subdivision

        if correct_DR_texture:
            V_sub, F_sub, tex_pred  = gu.subdivide_mesh_wrapper(igl.upsample, verts, faces, self.G_net.icosa_sphere_v, \
                                        self.G_net.icosa_sphere_f, self.G_net.albedo_predictor.uvimage_pred, subdiv_func.__name__ ) 

        else:
            V_sub, F_sub, tex_pred  = gu.subdivide_mesh_wrapper(subdiv_func, verts, faces, self.V_sphere, \
                                        self.G_net.icosa_sphere_f, self.G_net.albedo_predictor.uvimage_pred, subdiv_func.__name__ ) 

        default_texture_sub = torch.ones((1, F_sub.shape[1], 36, 3)).float().to(self.device)* color
        img_pred_shape_sub, _ = self.visual_renderer(V_sub, F_sub, default_texture_sub)
        #fix of strange bug, algthout V_sub, F_sub = V1t, F1t,  rendered image  differs from img_pred_shape1 in 25 pixels of values larger than 1 !!
        img_pred_shape_sub[img_pred_shape_sub > 1] = 1
        du.tensor_to_batch(SDR_to_numpy(img_pred_shape_sub), "img_pred_shape_sub")
        #img_pred_texture_sub, _ = self.visual_renderer(V_sub, F_sub, tex_pred.contiguous())
        img_pred_texture_sub, _ = self.visual_renderer(V_sub, F_sub, tex_pred.contiguous())
        
        img_pred_texture_sub[img_pred_texture_sub > 1] = 1
        du.tensor_to_batch( SDR_to_numpy(img_pred_texture_sub), "img_pred_texture_sub")
        print('')

        
        # V1t, F1t = gu.mesh2tensors( *igl.upsample(*gu.tensors2mesh(verts, faces)) )
        # default_texture_sub = torch.ones((1, F1t.shape[1], 36, 3)).float().to(self.device)* color
        # img_pred_shape1, _ = self.visual_renderer(V1t, F1t, default_texture_sub)
        # img_pred_shape1[img_pred_shape1 > 1] = 1
        # du.tensor_to_batch(img_pred_shape1, "img_pred_shape1")
   
        # VLt, FLt = gu.mesh2tensors( *igl.loop(*gu.tensors2mesh(verts, faces)) )
        # img_pred_shapeL, _ = self.visual_renderer(VLt, FLt, default_texture_sub)
        # img_pred_shapeL[img_pred_shapeL > 1] = 1
        # du.tensor_to_batch(img_pred_shapeL, "img_pred_shapeL")
    ...
    def save_results(self, save_dir, idx, verts, faces, texture, default_texture, Rt_pred, K_pred, scale_pred,
                     Rt_gt, K_gt, wms=False, class_name = ''):
        self.visual_renderer = SOFTRAS_renderer(img_size=self.args.img_size, camera_mode='projection',
                                                orig_size=self.args.img_size, background=[1, 1, 1],
                                                texture_type=self.args.texture_type, anti_aliasing=True,
                                                clamp_lighting=True)
        ...
        if not class_name: # valid for single class tests 
            class_name = os.path.split(save_dir)[-1].split("-")[0]

        DR_mode = None
        if self.save_mesh_in_test: 
            mean_shape_prefix = '_wms' if wms else ''  # mean_shape  if 'wms'=True, otherwise its predicted_mesh = mean_shape + predicted deform
            # if self.G_net.enable_symmetrization:
            #     symm_prefix = '_lr_sym' if self.G_net.enable_symmetrization == 1 else '_rl_sym'
            #     mean_shape_prefix = mean_shape_prefix + '_sym'
            DR_mode = {'save_obj': str(save_dir / 'obj' / f'{class_name + mean_shape_prefix}_{idx:04}.obj')} #pass obj file name to save mesh
        img_pred_texture, _ = self.visual_renderer(verts, faces, texture, DR_mode)#render predicted mesh with texture
        img_pred_texture = (img_pred_texture.squeeze(0).detach().cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
        self.visual_renderer.set_lighting(light_a_shape, light_d_shape, light_dir / torch.norm(light_dir))
        img_pred_shape, _ = self.visual_renderer(verts, faces, default_texture) #render predicted mesh without texture
        img_pred_shape = (img_pred_shape.squeeze(0).detach().cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)

        # V_sub, F_sub, tex_pred  = gu.subdivide_mesh_wrapper(igl.upsample, verts, faces, self.G_net.icosa_sphere_v, self.G_net.icosa_sphere_f, self.G_net.albedo_predictor.uvimage_pred) 
        # blue = torch.tensor([156, 199, 234.], device=self.device) / 255.
        # default_texture_sub =torch.ones((1, F_sub.shape[1], 36, 3)).float().to(self.device) 
        # img_pred_texture_sub, _ = self.visual_renderer(V_sub, F_sub, default_texture_sub)
        # du.tensor_to_batch(img_pred_texture_sub, "img_pred_texture_sub")

        # save obj mesh with unwrapped  texture    
        unwrapp_tex_tensor = self.G_net.albedo_predictor.uvimage_pred.cpu().detach()
        unwrapp_tex = (self._convert_texture(unwrapp_tex_tensor.permute(0, 2, 3, 1)).squeeze(0).numpy()*255).astype(np.uint8)

        V = verts.cpu().squeeze().numpy()
        # V_sphere = self.G_net.icosa_sphere_v.clone().cpu().numpy()
        # V_sphere[:,1] = - V_sphere[:,1]
        # F_sphere = self.G_net.icosa_sphere_f.clone().cpu().numpy()
        # from utils.geometry import save_obj_with_uv
        unwrapp_tex_file = f'{class_name}_uv_{idx:04}.png'
        # from utils.mesh import convert_3d_to_uv_coordinates
        # UV=(1-convert_3d_to_uv_coordinates(V_sphere))/2

        # #UV = convert_3d_to_uv_coordinates(V_sphere)
        cv2.imwrite(str(save_dir / 'unwrapped' / unwrapp_tex_file), unwrapp_tex[..., ::-1])
        obj_uv_file = './'  + str(save_dir / 'unwrapped' / f'{class_name + mean_shape_prefix}_uv_{idx:04}.obj',)
        V[:,1] = -V[:,1]
        #save_obj_with_uv(obj_uv_file,V, self.F_sphere, UV, unwrapp_tex_file)
        gu.save_obj_with_uv(obj_uv_file,V, self.F_sphere, self.UV, unwrapp_tex_file)
        print(obj_uv_file)
        # cv2.imwrite(str(save_dir / 'unwrapped' / f'{idx:04}.png'), unwrapp_tex[..., ::-1])
    ...
    def run_epoch(self, phase):
        ...
        #put it inside "if self.compute_symmetry_error:"
        num_of_samples= len(self.dataloader[phase]) 
        LR_mean = np.zeros(num_of_samples)
        LR_max  = np.zeros(num_of_samples)
        symmetry_str = ""
        V_right, _ ,V_opposite, V_middle = self.G_net.get_symmetry_indices()
        V_right_slice, V_left_slice      = self.G_net.get_symmetry_sliced_indices()
        
        cad_idx = -np.ones(num_of_samples).astype(int)
        ...
        for i, batch in enumerate(self.dataloader[phase]):
            ...
            #if phase == 'test':
            class_name_short = self.classes[ batch["class_idx"].tolist()[0] ]
            class_and_type = self.args.data_index_name.split('_')
            #class_and_type =os.path.split(self.save_dir)[-1].split("-")[0].split("_")
            class_name = class_name_short + ('_' + class_and_type[1] if len(class_and_type)>1 else '')


            # Get meanshape
            mean_shape = self.G_net.get_mean_shape() #mean shape is symmetric if  G_net.nable_symmetrization
            mean_shape = mean_shape.unsqueeze(0).expand(batch_size, -1, -1, -1)
            if self.save_all_mean_shape:
                import igl
                F = self.faces[phase].detach().cpu().squeeze(0).numpy()
                #class_name = os.path.split(self.save_dir)[-1].split("-")[0] #wrong for multiclass tests
                prefix_path =str(self.save_dir / 'obj' / f'{class_name}_{i:04}')
                for shape_i in range(mean_shape.shape[1]):
                    Vmean = mean_shape[0,i,:,:].clone().detach().cpu().numpy()
                    igl.write_obj(f'{prefix_path}_mean_mesh_{shape_i}.obj', Vmean, F)
                    print('saved ' + f'./{prefix_path}_mean_mesh_{shape_i}.obj')
            ...
            if phase != 'train':
                ...
                if phase == 'test' and self.args.save_results:
                    ...
                    input_Rt[:, :3, :3] = input_R
                    input_Rt[:, :, -1:] = input_tr.transpose(2, 1)
                    #computing left-rigth symmetry error (Only for SINGLE batch!)
                    #LR_diff = (mean_shape[0,V_right,:] - reflectX(mean_shape[0,V_opposite[V_right],:])).norm(dim=1) #mean-shape
                    #shape_scale = mean_shape[0,:,0].max() - mean_shape[0,:,0].min()
                    LR_diff = (pred_v[0,V_right,:] - gu.reflectX(pred_v[0,V_opposite[V_right],:])).norm(dim=1) #reconstructed mesh
                    shape_scale = pred_v[0,:,0].max() - pred_v[0,:,0].min()

                    LR_mean[i] =  ((LR_diff.sum()/ V_right.shape[0])/shape_scale).tolist()
                    LR_max[i]  =  (LR_diff.max()/shape_scale).tolist()
                    symmetry_str += f'Sample={i:04}: Mean-max Symmetry errors = ({LR_mean[i]:.4f}, {LR_max[i]:.4f})\n'
                    cad_idx[i] = batch['cad_idx'] 
                    #cad_class[i] = self.classes[ batch["class_idx"].tolist()[0] ]
                    ##print(f'Fitted CAD: {cad_class[i]}, index = {cad_idx[i]}, class_idx={batch["class_idx"].tolist()}')
                    

                    if self.args.qualitative_results:
                        #temporary !!!
                        enable_subdividion_test = False
                        if enable_subdividion_test:
                            self.test_subdivision(pred_v, self.faces[phase],self._convert_texture(albedo), Rt, intrinsic, scale_pred, input_Rt, input_intrinsic)

                        self.save_results(self.save_dir, i, pred_v, self.faces[phase],
                                          self._convert_texture(albedo),
                                          self.default_texture[phase],
                                          Rt, intrinsic, scale_pred, input_Rt, input_intrinsic, False, class_name)
                        self.save_results(self.save_dir, i, mean_shape, self.faces[phase],
                                          self._convert_texture(albedo),
                                          self.default_texture[phase],
                                          Rt, intrinsic, scale_pred, input_Rt, input_intrinsic, True,  class_name)
                    else:
                        self.save_results(self.save_dir, i, pred_v, self.faces[phase],
                                          self._convert_texture(albedo),
                                          self.default_texture[phase],
                                          Rt, intrinsic, scale_pred, input_Rt, input_intrinsic, False, class_name)
                    if self.save_input_in_test:
                        #class_name = os.path.split(self.save_dir)[-1].split("-")[0] #wrong for multiclass tests 
                        cv2.imwrite(str(self.save_dir / 'input' /  f'{class_name}_rgb_{i:04}.png'),
                                        cv2.cvtColor(du.torch_image_to_numpy(batch['image'])*255,cv2.COLOR_RGB2BGR))
                        cv2.imwrite(str(self.save_dir / 'input' /  f'{class_name}_mask_{i:04}.png'), 
                                        du.torch_image_to_numpy(batch['mask'])*255)
            ...
            # Compute losses
            mask_loss = self.mask_loss_fn(mask_pred, input_masks) #mask_pred = not black pixels of rendered mesh 
            ...
            total_loss = 0.
            total_loss += self.args.mask_loss_wt * mask_loss # loss between gt maks and mask obtianed by collecting blank pixel from rendered images
            total_loss += self.args.deform_reg_wt * deform_reg                  # shape regularization losses
            total_loss += self.args.laplacian_wt * laplacian_loss               # (where is the symmetry loss)
            ...
            self.losses['Graph Laplacian Loss'][epoch_iter] = graph_laplacian_loss.item() #how it differs from the previous loss?
            ...
            self.losses['Color Loss'][epoch_iter] = tex_color_loss.item() #standard_image_loss(masked input,  masked rendering of predicted mesh with texture)  
        ...
        if phase == 'test':
            ...
            symmetry_str += '=====================================================================================\n'
            symmetry_str += f'Average Mean-Max symmetry errors per sample =  ({LR_mean.sum()/num_of_samples:.4f}, {LR_max.sum()/num_of_samples:.4f})\n'
            symmetry_str += f'Max Max-Symmetry Errors  =  {LR_max.max():.4f}  for sample {LR_max.argmax():04}\n'
            print(symmetry_str)
            with open(str(self.save_dir / 'shape_symmetries.txt'), 'w') as fd:
                    fd.write(symmetry_str)
                    fd.close()

            symmetry_data = {'LR_mean':LR_mean, 'LR_max':LR_max,
                            'V_right':V_right,'V_opposite':V_opposite,
                            'V_middle':V_middle, 'cad_idx':cad_idx,
                            'V_right_slice':V_right_slice,
                            'V_left_slice': V_left_slice,
                            }
            
            np.save(str(self.save_dir / 'shape_symmetries.npy'), \
                        symmetry_data, \
                        allow_pickle =True )
        ...
        if not self.args.qualitative_results:
            ...
            if phase == 'val':
                if self.losses['IoU Metric (pred cam)'].mean() > self.best_IoU:
                    ...
                    self.save_checkpoint(save_path)
        if  phase == 'test':
            with open(f'{self.save_dir}/test_results.pkl', 'wb') as fd: #summury of all results 
                    pickle.dump( {"losses":self.losses, "args":self.args, 'symmetry_data':symmetry_data}, fd)

    def init_train(self):
        self.args.sdf_subdivide_steps = [int(x) for x in str(args.sdf_subdivide_steps).split(',')] \
            if args.sdf_subdivide_steps != '' else []

        self.log_arguments()  # args <-- parsed cmd line  arguments 
        self.define_dataset() #torch loader of train dataset
        self.define_model()
        self.define_renderer()
        self.define_criterion()
    ...
    def test(self):
        ...
        self.eval_idxs = np.sort(np.random.choice(self.eval_set_idxs, min(64, len(self.eval_set_idxs)), replace=False))
        ...
        if self.args.save_results:
            if self.args.dataset_name == 'cub':
                ...
                for i, class_name in enumerate(self.classes):
                    if i == len(self.classes) - 1:
                        dir_name += f'{class_name}{self.args.data_index_name}'
                    else:
                        dir_name += f'{class_name}{self.args.data_index_name}-'
                if self.args.qualitative_results:
            ...
            if self.save_mesh_in_test:
                sub_dirs.append('obj')
                sub_dirs.append('unwrapped')
            if self.save_input_in_test:
                sub_dirs.append('input')
        ...
        with torch.no_grad(): #set requires_grad=False for all tensors in MCMR 
...
if __name__ == '__main__':
    ...
    #additional configs for data 
    parser.add_argument('--data_index_name', type=str, default = '', help= 'Data Index file is <--class>_<--data_index_name><test/train/eval>.txt')
    #parser.add_argument('--symmetrize', type=int, help='enable  different types of mesh symmetrization', default = 0)
    parser.add_argument('--symmetrize', type=int, nargs='+', help='enable  different types of mesh symmetrization', default = "0")

    
    args = parser.parse_args()

    # args.symmetrize =  [int(val) for val in args.symmetrize] #string to ints
    # if  len(args.symmetrize)==1:
    #     args.symmetrize = args.symmetrize[0]
   
    #cmd argument simplifications 
    if args.num_learned_shapes == 1: 
        args.single_mean_shape = True
    if len(args.classes) > 0:  
        args.sub_classes = False
    ...
    now = str(datetime.now().strftime("%m.%d.%Y_%H-%M-%S"))  #remove special characters because its log dir name   
    print(now)

    #custom txt indexing to allow a test/train on a samll subsets 
    if args.data_index_name == '_': # '_' means the same as empty argument, i.e. use the default data indexing
       args.data_index_name = ''    # (change '_' to 'all')
    if args.data_index_name: 
        args.data_index_name = '_' + args.data_index_name
    
    #add values from pxi2surf code that I ran on the same machine 
    os.environ["CUDA_VISIBLE_DEVICES"] = '0, 1' #should be passed as command line argumnet !!!
    print(f'CUDA_VISIBLE_DEVICES: {os.environ["CUDA_VISIBLE_DEVICES"]}')

    #self.args.G_lr_steps.index(self.epoch)
    multi_shape_predictor = MultiShapePredictor(args, DEBUG, now)
    
    #converting int values to range list, otherwise it fails 
    # if type(multi_shape_predictor.args.G_lr_steps) == int:
    #     multi_shape_predictor.args.G_lr_steps = list(range(args.G_lr_steps))
    # if type(multi_shape_predictor.args.G_lr_steps_values) != list:
    #     multi_shape_predictor.args.G_lr_steps_values = [args.G_lr_steps_values] * len(multi_shape_predictor.args.G_lr_steps)

    if type(multi_shape_predictor.args.G_lr_steps) != list:
        multi_shape_predictor.args.G_lr_steps = [multi_shape_predictor.args.G_lr_steps]
    if type(multi_shape_predictor.args.G_lr_steps_values) != list:
        multi_shape_predictor.args.G_lr_steps_values = [multi_shape_predictor.args.G_lr_steps_values]
