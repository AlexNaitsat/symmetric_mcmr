...
class PascalDataset(Dataset):
    ...
    def __init__(self, dataset_dir: Path, classes: list, img_size: int = 224, init_mode: str = 'train',
                 demo_mode: bool = False, aug_mode: bool = False, cmr_mode: bool = False, data_index_name: str = ''):
        ...
        for class_sub_dir in sorted(self.dataset_dir.glob('*/')):
            if class_sub_dir.stem in self.classes:
                if self.cmr_mode and split == 'test' and class_sub_dir.stem == 'car':
                    ...
                    car_val_mat_path  = '/home/ubuntu/research/datasets/UCMR_P3D_data/p3d/data/car_val.mat' 
                    test_UCMR = loadmat(car_val_mat_path,
                                        struct_as_record=False, squeeze_me=True)['images']
                    test_UCMR_bbox = [[x.bbox.x1, x.bbox.y1, x.bbox.x2, x.bbox.y2]
                                      for x in loadmat(car_val_mat_path,
                                                       struct_as_record=False, squeeze_me=True)['images']]

                meta_filenames = np.loadtxt(str(class_sub_dir / f'{class_sub_dir.stem}{self.data_index_name}_{split}.txt'), dtype=str)
                if self.demo_mode:
                    meta_filenames = meta_filenames[:min(len(meta_filenames),100)]
                
                #iterate over yaml files from "<class>_test.txt"    
                for meta_f in tqdm(meta_filenames, f'[{self.name}] Loading {class_sub_dir.stem} {split} set'):
                    ...
                    if self.cmr_mode and split == 'test' and class_sub_dir.stem == 'car': #car masks are taken from the mat file 
                        #car meta data is saved  in  two files:  mask and  bbox (test_UCMR_bbox) saved in  "car_val.mat", 
                        # bbox (img_annotations['bbox']) and other data are  saved in  yaml files from "car_test.txt"
                        if img_annotations['bbox'] not in test_UCMR_bbox: #find mat index of the current car by looking for mat bboxes for the current yaml bbox
                            continue                                      #if not found then the car was not annotated in "car_val.mat", so skip it 
                        mask_idx = test_UCMR_bbox.index(img_annotations['bbox']) 
                        mask = test_UCMR[mask_idx].mask * 255
                    else: #for other classes masks are taken form preprocessed PAscak3D+ mask images  
                        mask_paths = [
                            'masks_VOC',
                            'masks_hariharan',
                            'masks_cmr',
                    ...
                    #the rest of data is taken from yaml annotations
                    if (mask != 0).sum() == 0:
                        continue
                    ...
                    cad_idx = img_annotations['cad_idx'] #index of CAd model used to find a coarse pixel-to-3D correspondence ?

                    kpoints = np.asarray(img_annotations['kpoints_2d'], dtype=np.float32) #keypoints used to fit CAD model to image and to get rought camera pose ?
                    ...
                    sample = { #data stored for __get_item__ fetching 
                    ...
                    #debug check of annotation
                    if debug_outputs:
                        im_ = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
                        im_masked = im_ * np.tile(mask[:,:,np.newaxis]/mask.max(), (1,1,3))
                        du.horizontal_pictures_with_title('./debug/'+os.path.split(image_path)[-1].split('.')[0]+'_mask_check.png',
                                                        {'image': im_, 'mask': mask, 'masked': im_masked.astype(int),
                                                         'image_bbox':im_[bbox[1]:bbox[3],bbox[0]:bbox[2],]
                                                        }, 
                                                       f"{path}/{meta_f.stem}.png of {img_annotations['image_name']} from {os.path.split(meta_f)[-1]} {split} mode",
                                                        batch = None
                                                        )
